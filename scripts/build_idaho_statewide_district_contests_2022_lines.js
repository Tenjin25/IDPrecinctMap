...
const OUT_2022_DIR = path.join(DATA_DIR, 'district_contests_2022_lines');
const OUT_2026_DIR = path.join(DATA_DIR, 'district_contests_2026_lines');
const MANIFEST_2022_PATH = path.join(OUT_2022_DIR, 'manifest.json');
...
function normalizeDistrictLinks(links) {
  const totalsByDistrict = new Map();
  for (const link of links || []) {
    const districtNum = Number(link?.districtNum);
    const weight = Number(link?.weight);
    if (!Number.isFinite(districtNum) || !Number.isFinite(weight) || weight <= 0) continue;
    totalsByDistrict.set(districtNum, (totalsByDistrict.get(districtNum) || 0) + weight);
  }

  const normalized = Array.from(totalsByDistrict.entries())
    .map(([districtNum, weight]) => ({ districtNum, weight }))
  ...
  if (totalWeight > 1.000001) {
    for (const link of normalized) {
      link.weight = link.weight / totalWeight;
    }
  }
...
function main() {
  ...
  const manifest2022 = loadManifest(MANIFEST_2022_PATH);
  const manifest2026 = loadManifest(MANIFEST_2026_PATH);
  const new2022Entries = [];
  const new2026Entries = [];

  manifest2022.files = filterExistingEntries(manifest2022);
  ...
  writeJson(MANIFEST_2022_PATH, manifest2022);
